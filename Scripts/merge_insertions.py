        ...
        for line in insertions:
            ...
            if ins_chrom == "ins_chr":
                ...
                while x <= i:
                    ...
                    if (
                        master_dict[x]["ins_chrom"] == ins_chrom
                        and same_te is True
                        and ref_chrom == master_dict[x]["ref_chrom"]
                        and ref_start == master_dict[x]["ref_start"]
                    ):
                        # does it overlap with the insertion coordinated for current accession
                        if (
                            overlap(
                                master_dict[x]["ins_start"],
                                master_dict[x]["ins_end"],
                                ins_start,
                                ins_end,
                            )
                            is True
...
def main(filename):
        ...
        if os.path.isdir(dirs) is True:
            ...
            if os.path.isfile(filename + "_{d}.bed".format(d=dirs)) is True:
                ...
                try:
                    ...
                    master_insertions = create_master_dict(
                        filename + "_{d}.bed".format(d=dirs), dirs
                    )
                else:
                    merge_insertions(
                        master_insertions, filename + "_{d}.bed".format(d=dirs), dirs
                    )
                os.chdir("..")
            else:
                os.chdir("..")
    ...
    with open(filename + ".bed", "w+") as outfile:
        for key, value in master_insertions.items():
            ...
            outfile.write(
                """{ins_chrom}\t{ins_start}\t{ins_end}\t{ref_chrom}\t{ref_start}\t{ref_end}\t{agi}\t{accessions}\n""".format(
                    ins_chrom=value["ins_chrom"],
...
parser.add_argument(
    "-f", "--filename", help="filename prefix for merge files", required=True
)
